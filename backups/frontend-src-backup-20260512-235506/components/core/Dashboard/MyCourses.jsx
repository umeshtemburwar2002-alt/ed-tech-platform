...
import {
  ...
  FaFilter, FaStar, FaUsers, FaCheckCircle, FaGlobe, FaLock
} from "react-icons/fa";
import { getInstructorCourses, deleteCourse as supabaseDeleteCourse, setCourseStatus } from "../../../services/courseService";
      ...
      <td className="py-5 px-6">
        <div className="flex items-center gap-4">
          <div className="w-20 aspect-video rounded-lg overflow-hidden bg-richblack-900 border border-richblack-700 shrink-0">
            <img src={course.thumbnail} alt={course.courseName} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
          </div>
          <div className="min-w-0">
            <p className="font-bold text-richblack-5 line-clamp-1 text-sm group-hover:text-yellow-50 transition-colors">{course.courseName}</p>
...
export default function MyCourses() {
  ...
  // modal state
  const [viewCourse,   setViewCourse]   = useState(null); // show CourseViewModal
  const [editCourse,   setEditCourse]   = useState(null); // show CourseEditModal
  const [deleteCourse, setDeleteCourse] = useState(null); // show DeleteModal
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error) {
        ...
        console.error("[MyCourses] Supabase error:", error);

        // Map common Supabase error codes to helpful messages
        let msg = "Failed to load courses";
        if (error.code === "42P01")
          msg = "Database table missing. Run complete_schema.sql in Supabase SQL Editor.";
        else if (error.code === "PGRST116" || error.message?.includes("RLS"))
          msg = "Access denied (RLS). Run phase2_patch.sql in Supabase SQL Editor.";
        else if (error.message?.includes("Failed to fetch") || error.message?.includes("network"))
          msg = "Network error. Check your REACT_APP_SUPABASE_URL in .env";
        else
          msg = error.message || "Failed to load courses";

        setFetchError(msg);
        toast.error(msg, { duration: 6000 });
      } else {
        // Normalize shape to match existing CourseRow expectations
  ...
  const handleConfirmDelete = async () => {
    ...
    if (error) {
      ...
      toast.success(`"${deleteCourse.courseName}" deleted`);
    }
      ...
      {fetchError && (
        <div className="flex items-start gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/30 text-sm">
          ...
          <button
            ...
            className="text-red-400/50 hover:text-red-400 text-lg shrink-0 ml-auto"
          >×</button>
        </div>
      )}
