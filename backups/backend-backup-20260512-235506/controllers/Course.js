...
exports.getAllCourses = async (req, res) => {
    try {
        const { data: allCourses, error } = await supabase
            .from('courses')
            .select('*, instructor:instructor_id(first_name, last_name, image)')
            .eq('status', 'Published');
...
exports.editCourse = async (req, res) => {
    try {
        ...
        if (updates.whatYouWillLearn)  patch.what_you_will_learn = updates.whatYouWillLearn;
        if (updates.tag)               patch.tags               = typeof updates.tag === "string" ? JSON.parse(updates.tag) : updates.tag;
        if (updates.category)          patch.category_id        = updates.category;
        if (updates.status)            patch.status             = updates.status;
        if (updates.instructions)      patch.instructions       = typeof updates.instructions === "string" ? JSON.parse(updates.instructions) : updates.instructions;
